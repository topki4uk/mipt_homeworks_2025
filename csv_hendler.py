...
class DataProcessor:
    ...
    def group_by(self, field: str, aggregation: Optional[Dict[str, Callable]] = None) -> 'DataProcessor':
        """
        ...
        self._validate_fields([field])
        if aggregation:
            self._validate_fields(aggregation.keys())
    ...
    def _validate_fields(self, fields: List[str]) -> None:
        ...
        for field in fields:
            if field not in self.available_fields:
                similar = get_close_matches(field, self.available_fields, n=3, cutoff=0.6)
    ...
    def _apply_select(self, data: List[Dict[str, Any]], fields: List[str]) -> List[Dict[str, Any]]:
        """Применение операции SELECT"""
        return [{field: item.get(field) for field in fields} for item in data]

    def _apply_where(self, data: List[Dict[str, Any]], condition: Callable) -> List[Dict[str, Any]]:
        """Применение операции WHERE"""
        return [item for item in data if condition(item)]

    def _apply_sort(self, data: List[Dict[str, Any]], field: str, reverse: bool = False) -> List[Dict[str, Any]]:
        """Применение операции SORT"""
        return sorted(data, key=itemgetter(field), reverse=reverse)

    def _apply_group_by(self, data: List[Dict[str, Any]], field: str, aggregation: Dict[str, Callable]) -> List[
        Dict[str, Any]]:
        ...
        for group_key, group_items in groups.items():
            ...
            for agg_field, agg_func in aggregation.items():
                values = [item.get(agg_field) for item in group_items if item.get(agg_field) is not None]
                if values:
    ...
    def execute(self) -> List[Dict[str, Any]]:
        ...
        for operation in optimized_ops:
            ...
            try:
                ...
                raise RuntimeError(f"Ошибка при выполнении операции {op_type}: {str(e)}")
...
if __name__ == "__main__":
    # Пример данных (репозитории GitHub)
    sample_data = [
        {
            "Name": "django", "Language": "Python", "Stars": 75000, "Forks": 32000,
            "Size": 250, "Has Issues": True, "Is Fork": False
        },
        {
            "Name": "react", "Language": "JavaScript", "Stars": 210000, "Forks": 45000,
            "Size": 180, "Has Issues": True, "Is Fork": False
        },
        {
            "Name": "tensorflow", "Language": "Python", "Stars": 180000, "Forks": 88000,
            "Size": 420, "Has Issues": True, "Is Fork": False
        },
        {
            "Name": "vue", "Language": "JavaScript", "Stars": 205000, "Forks": 34000,
            "Size": 150, "Has Issues": True, "Is Fork": False
        }
    ]
    ...
    try:
        # Сложный запрос с несколькими операциями
        result = (processor
                  .select(["Name", "Language", "Stars", "Forks"])
                  .where(lambda x: x["Stars"] > 100000)
        ...
        processor2 = DataProcessor(sample_data)
        result2 = (processor2
                   .group_by("Language", {"Stars": avg, "Forks": sum_agg})
                   .execute())

        print("Группировка по языку:")
        for item in result2:
            print(item)
        ...
        # План выполнения
        processor3 = DataProcessor(sample_data)
        plan = (processor3
                .where(lambda x: x["Stars"] > 50000)
                .sort("Stars")
                .select(["Name", "Stars"])
                .limit(10)
                .explain())
    ...
    try:
        processor4 = DataProcessor(sample_data)
        result4 = processor4.sort("Starrs").execute()  # Опечатка в названии поля
    except ValueError as e:
